...
func (vu *ViewUpdater) executeSQL(db *sql.DB, sqlStr string) error {
    // 使用统一重试机制执行，符合错误处理设计要求
    if err := retry.ExecWithRetryDefault(db, vu.config, sqlStr); err != nil {
...
func (vu *ViewUpdater) UpdateSingleView(srDB, chDB *sql.DB, dbManager *database.DatabasePairManager, pair config.DatabasePair, viewName string) error {
    ...
    if vu.config != nil && vu.config.TimestampColumns != nil {
        if cfg, ok := vu.config.TimestampColumns[srTableName]; ok {
            if cfg.Column != "" { tsCol = cfg.Column }
            if cfg.Type != "" { tsType = cfg.Type }
    ...
    // 委托一次性更新库执行（显式传分区值）
    return updaterun.UpdateSingleView(vu.config, srDB, chDB, dbManager, pair, viewName, partStr, true, isNumeric)
}
...
// 解析函数改为复用 internal/common.ParseTableFromString
