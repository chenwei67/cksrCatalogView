...
func (dm *DatabasePairManager) Init() error {
	...
	if err := dm.InitStarRocks(); err != nil {
		return err
	}
	return nil
}

// InitClickHouse 仅初始化 ClickHouse 连接。
func (dm *DatabasePairManager) InitClickHouse() error {
	if dm.clickhouseDB != nil {
		return nil
	}
...
// InitStarRocks 仅初始化 StarRocks 连接。
func (dm *DatabasePairManager) InitStarRocks() error {
	if dm.starrocksDB != nil {
		return nil
	}
...
type StarRocksColumnSchema struct {
	ColumnName           string
	OrdinalPosition      int
	ColumnType           string
...
// GetStarRocksTableSchema 通过 information_schema.columns 获取 StarRocks 表结构。
func (dm *DatabasePairManager) GetStarRocksTableSchema(tableName string) (parser.Table, error) {
	db, err := dm.GetStarRocksConnection()
	if err != nil {
		return parser.Table{}, fmt.Errorf("获取StarRocks连接失败: %w", err)
	}
	...
	if err != nil {
		return parser.Table{}, fmt.Errorf("查询StarRocks表 %s 列信息失败: %w", tableName, err)
	}
	defer rows.Close()

	var columns []StarRocksColumnSchema
	for rows.Next() {
		var column StarRocksColumnSchema
		if err := rows.Scan(
			&column.ColumnName,
			...
			return parser.Table{}, fmt.Errorf("扫描StarRocks表 %s 列信息失败: %w", tableName, err)
		}
		columns = append(columns, column)
	}
	if err := rows.Err(); err != nil {
		return parser.Table{}, fmt.Errorf("遍历StarRocks表 %s 列信息失败: %w", tableName, err)
	}

	return buildStarRocksTableSchema(pair.StarRocks.Database, tableName, columns)
}

func buildStarRocksTableSchema(dbName, tableName string, columns []StarRocksColumnSchema) (parser.Table, error) {
	if len(columns) == 0 {
		return parser.Table{}, fmt.Errorf("StarRocks表 %s.%s 未查询到任何列", dbName, tableName)
	}
